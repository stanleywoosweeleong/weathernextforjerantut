...
locations around Jerantut, Pahang. Multilingual interface
...
This release migrates Jerantut from the older forecast-only build onto
...
```
https://stanleywoosweeleong.github.io/weathernextforjerantut/
```
...
|---|---|---|---|---|
| Kg. Simpang Tebing Tinggi | 德宾丁宜新村 | 3.87467, 102.34467 | open plain | durian |
| Damak | 达玛 | 3.949544, 102.214385 | open plain | durian |
| Bukit Seladang | 友联港神山 | 3.91313, 102.35476 | sheltered basin | durian |
...
The app also seeds a default user display name (**CR**),
which stays editable via **Edit Name** in the app.

This build carries the seed version **`jt-arch1`** (the lowland-architecture
...
```
wnext-weathernextforjerantut-202606062246
```
...
- **Storage namespace:** `weathernextforjerantut__*` keys in `localStorage`, isolated
  from other WeatherNext regional builds so data never collides.
- **Cloud sync:** Firebase, namespaced under `appId: wnext-ag-v41-weathernextforjerantut`.
...
The icon shows a farmer in an orchard under a blue sky. The boot screen
background (sky-blue `#8dcdf3`) is matched to the icon's sky.
